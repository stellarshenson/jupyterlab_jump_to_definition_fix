...
After editing Python files (like `jupyterlab_jump_to_definition_fix/routes.py`):

```bash
python -m py_compile jupyterlab_jump_to_definition_fix/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyterlab_jump_to_definition_fix/*.py` files):
...
- Backend processing in `jupyterlab_jump_to_definition_fix/routes.py`
...
- **✅ Do**: `jupyterlab_jump_to_definition_fix/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyterlab_jump_to_definition_fix:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyterlab_jump_to_definition_fix:open-panel';
  export const refreshData = 'jupyterlab_jump_to_definition_fix:refresh-data';
}
...
- Backend routes: `jupyterlab_jump_to_definition_fix/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyterlab_jump_to_definition_fix/` directory
...
1. **Read the backend first** — Check `jupyterlab_jump_to_definition_fix/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyterlab_jump_to_definition_fix/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyterlab_jump_to_definition_fix", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyterlab_jump_to_definition_fix/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyterlab_jump_to_definition_fix/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable jupyterlab_jump_to_definition_fix
   ```
...
- **Output**: Creates files in `lib/` and `jupyterlab_jump_to_definition_fix/labextension/`
...
**After editing Python** (files in `jupyterlab_jump_to_definition_fix/`):
...
- Ask user to search for the extension ID (`jupyterlab_jump_to_definition_fix`) to see if it loaded
...
ls -la jupyterlab_jump_to_definition_fix/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyterlab_jump_to_definition_fix/routes.py`
- Server extension config: `jupyter-config/server-config/jupyterlab_jump_to_definition_fix.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyterlab_jump_to_definition_fix/routes.py`
...
```css
.jp-jupyterlab-jump-to-definition-fix-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from jupyterlab_jump_to_definition_fix.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyterlab_jump_to_definition_fix:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterlab_jump_to_definition_fix:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterlab-jump-to-definition-fix-ClassName`
- **API routes** (in `jupyterlab_jump_to_definition_fix/routes.py`): `url_path_join(base_url, "jupyterlab_jump_to_definition_fix", "endpoint")`
